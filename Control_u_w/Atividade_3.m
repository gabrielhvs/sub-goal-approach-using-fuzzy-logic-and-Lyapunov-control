...
%%    
    ...
    GS = readfis('Goal_seeking');
    OA = readfis('Obtacle_avoidance');
    TR = readfis('Tranking');
    DLo = readfis('DeadLoak');
    distanceS = zeros(16);
...
%%
    for i=1:Np
        ...
        DL = min(distanceS(1),distanceS(16));
        DF = min(distanceS(4:5));
        DR = min(distanceS(8:9));
